function [] = heat_equation_1D_cn_scheme_example_4
%%% --- 1D heat equation 
%
% du/dt = D*d^2u/dx^2 - 4*x*(1-x)*u + e^(-t)*(8*x^2-t) with BC: u(0,t) = 0.
%                                                      u(1,t) = 8*t*exp(-t)
%                                                      u(x,0) = 2*sin(2*pi*x)
% analytic solution:
%
...
%  matrix equation: 
% (I-lambda*A -0.5*dt*B^(n+1))*w^(n+1) = (I+lambda*A)*w^(n) + lambda*(b^(n) + b^(n+1)) + 0.5*dt*(s^(n)+s^(n+1))
% A is tridiagonal matrix.
% (I-lambda*A) is evolution matrix.
% B is diagonal matrix with [-4.*x.*(1.-x)].
...
% evolution matrix 
B_mat = -4.*x.*(1.-x); 
evolution_mat_l = unit_I - u_mat - 0.5.*dt.*diag(B_mat);
evolution_mat_r = unit_I + u_mat + 0.5.*dt.*diag(B_mat);
%
...
for j = 1:Nt
    tn(j) = ti + (j-1)*dt; % t(n)
    bn(j) = 8.*tn(j).*exp(-tn(j));    
    b_n = [0.; zeros(N-3,1); bn(j)];
    s_n = exp(-tn(j)).*(8.*x(2:N).^2 - tn(j));
%
    tnp1(j) = ti + (j-0)*dt; % t(n + 1) 
    bnp1(j) = 8.*tnp1(j).*exp(-tnp1(j));    
    b_np1 = [0.; zeros(N-3,1); bnp1(j)];
    s_np1 = exp(-tnp1(j)).*(8.*x(2:N).^2 - tnp1(j));
%
    w_old = w0;
