function [] = heat_equation_1D_ftcs_scheme
...
%
% The finite difference: the forward-time centered space (FTCS), that is,
%  matrix equation: 
% w^(n+1) = (I+lambda*A)*w^(n) + lambda*b^(n).
% A is tridiagonal matrix.
% (I+lambda*A) is evolution matrix.
% b^(n) = [u^(n)_{a} 0,...0, u^(n)_{b}]^(T), (T = transpose). 
% b^(n) = 0 for all n since u(a,t) = (b,t) = 0
% lambda = D*dt/dx^2.
...
%
    w_old = w0;
    w_new = evolution_mat(2:N,2:N) * w_old; % % FTCS scheme
    w0 = w_new;
    %
end
%%% time loop ends ---
